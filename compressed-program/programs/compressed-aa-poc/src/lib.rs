...
pub mod compressed_aa_poc {
    ...
    pub fn exec_multiple_instructions<'info>(
        ctx: LightContext<'_, '_, '_, 'info, ExecMultipleInstructions<'info>>,
        ...
        for (_i, data) in instruction_data.iter().enumerate() {
            let verve_instruction: VerveInstruction = VerveInstruction::try_from_slice(&data)?;

            let mut account_metas: Vec<AccountMeta> = vec![];

            for (j, account_index) in verve_instruction.account_indices.iter().enumerate() {
                let account_key: Pubkey = ctx.remaining_accounts[*account_index as usize].key();
                let is_writable: bool = verve_instruction.writable_accounts[j];
                let is_signer: bool = verve_instruction.signer_accounts[j];
...
#[light_accounts]
pub struct ExecMultipleInstructions<'info> {
    /// CHECK: No need to have this account initialized lol
    #[account(
        seeds=[PDA_WALLET_SEED, seed_guardian.key().as_ref()],
        bump
